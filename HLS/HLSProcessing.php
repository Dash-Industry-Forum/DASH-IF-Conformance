...
function processHLS()
{
    global $session, $mpd_url;
    global $mpdHandler;
    global $modules;
    global $logger;

    $mpdHandler = new DASHIF\MPDHandler(null);

    global $hls_manifest;
    $hls_manifest = true;


    global $mpd_features,                                   // Client block input
    ...
    $progress_xml->asXml($session->getDir() . '/' . $progress_report);
    ...
    if ($m3u8) {
        ...
        fwrite(STDERR, "Ran beforerepresentation hooks\n");
        ...
        fwrite(STDERR, "Formed mpd features\n");
...
{
    if (DASHIF\Utility\isAbsoluteURL($line)) {
        return $line;
    }
...
{
    global $session, $hls_current_index, $hls_byte_range_begin,
    $hls_byte_range_size, $progress_xml, $progress_report;
    ...
    foreach ($urlarray as $url) {
        ...
        rename($session->getSelectedRepresentationDir() . '/mdatoffset', $session->getDir() . '/' . $type . '/' . $hls_current_index . '/mdatoffset');
        if ($type == 'IFrameByteRange') {
...
{
    global $session, $hls_media_types, $adaptation_set_template, $reprsentation_template, $progress_xml, $progress_report, $string_info;
    ...
    $i = 0;
    $file_location_ind = 0;
    $period_dir = $session->getPeriodDir(0);
    ...
    foreach ($hls_media_types as $hls_media_type) {
        foreach ($hls_media_type as $sw) {
            ...
            foreach ($sw as $track) {
                ...
                $pathdir = $name[0];
                $pathdir_ind = $name[1];
                $track_path = $session->getDir() . '/' . $pathdir . '/' . $pathdir_ind;
                ...
                rename($session->getDir() . '/' . $track . 'log.txt', $period_dir . '/' . $new_track_path . 'log.txt');
