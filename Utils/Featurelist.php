...
{
    public const INSERT_NEW_CHILD = 1;
    public const SWITCH_TO_SIBLING = 2;
    public const CHANGE_PARENT = 3;
}
                            ...
                            while (true) {
                                ...
                                if ((strpos($myName, "#") === false) and ! empty($myName)) {
                                    ...
                                    formCurrentXmlPath(
                                        $pathOfElement,
                                        $myName,
                                        PathBuildActionTypes::SWITCH_TO_SIBLING
                                    );
                                    setFeatureListEntry(
                                        $SubNode,
                                        $pathOfElement,
...
/* ! \fn formCurrentXmlPath(&$path,$nodeName, $actionType)
  \brief iteratively builds the current element path ($path) as a string containing the path currently evaluated.
  The path is build concatenanting the element name (e.g. /MPD[1]/Period[1]/AdaptationSet[2]/Representation[1] ).
...
/* ! \fn isElementWithSchemaIssues($currenElementPath,$schematronReport )
  ...
  \param $schematronReport is the string/character that signals the init of the data that will be extracted.
  \return returns an $schemaIssues object that contains two elements: $schemaIssues->issue that is equal to "false"
  if no issue was found, otherwise, it contains the text of the issue, and $schemaIssues->attributes that contains
  an array of all attributes that are affected by the issue, if present.
        ...
        if ($myName !== "#text" and ! empty($myName)) {
            ...
            if ($numbOfAttributes === 0) {
                ...
                for ($i = 0; $i < $numbOfAttributes; ++$i) {
                    ...
                    } elseif ($firstFalseValueSet === false) {
...
function populateList($xml, $html_str)
{
    ...
    foreach ($attributes as $attribute) {
        ...
        if (
            strpos($attribute->name, 'xmlns') !== false ||
