...
abstract class InitSegmentComponent extends ModuleComponent
{
    ...
    {
        $cachePath = cache_path([$this->component, $representation->path(), 'init']);
        $serialized = Cache::rememberForever(
            $cachePath,
            function () use ($representation, $segment) {
                $this->validateInitSegment($representation, $segment);
