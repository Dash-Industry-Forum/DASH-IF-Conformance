...
abstract class SegmentListComponent extends ModuleComponent
{
    ...
    {
        $cachePath = cache_path([$this->component, $representation->path()]);
        $serialized = Cache::rememberForever(
            $cachePath,
            function () use ($representation, $segments) {
                $this->validateSegmentList($representation, $segments);
