...
<schema xmlns="http://purl.oclc.org/dsdl/schematron" xmlns:dash="urn:mpeg:dash:schema:mpd:2011" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xlink="http://www.w3.org/1999/xlink" queryBinding='xslt' schemaVersion='ISO19757-3'>
	...
	<pattern name="AdaptationSet element">
		...
		<rule context="dash:AdaptationSet">
			...
			<!-- RD3.7	DASH-IF -->
			<assert test="if (@mimeType and not((@mimeType = 'video/mp4') or (@mimeType = 'audio/mp4') or (@mimeType = 'application/mp4') or (@mimeType = 'application/ttml+xml') or (@mimeType = 'text/vtt') or (@mimeType = 'image/jpeg'))) then false() else true()">If a DASH-IF profile identifier is present, for the Adaptation Sets the mimeType shall be one of the six following type: "video/mp4", "audio/mp4", "application/mp4", "application/ttml+xml", "text/vtt" or "image/jpeg"</assert >
	...
	<pattern name="Representation element">
		...
		<rule context="dash:Representation">
			...
			<assert test="if ((@id = preceding-sibling::dash:Representation/@id) or (@id=parent::dash:AdaptationSet/preceding-sibling::dash:AdaptationSet/dash:Representation/@id))then false() else true()">The id of each Representation within a Period shall be unique.</assert>
                        <!--Dual-Stream Dolby Vision check-->
                        <assert test="if(@dependencyId and not(@dependencyId = preceding-sibling::dash:Representation/@id))then false() else true()">The @dependencyId attribute on the Enhancement Layer Representation shall refer to the Base Layer Representation @id attribute.</assert>
                        <!-- DASH-IF Section 5.3.7.2 -->
	...
	<pattern name="AudioChannelConfiguration element">
		...
		<rule context="dash:AudioChannelConfiguration">
			...
			<assert test="if ((@schemeIdUri = 'urn:mpeg:dash:outputChannelPositionList:2012') and not(count(tokenize(@value, ' ')) > 1)) then false() else true()">If URI urn:mpeg:dash:outputChannelPositionList:2012 is used the value attribute shall be a space-delimited list as defined in ISO/IEC 23001-8.</assert>
                        ...
                        <assert test="if (contains(ancestor::dash:MPD/@profiles, 'http://dashif.org/guidelines/dashif#mha1') and not(@schemeIdUri='urn:mpeg:mpegB:cicp:ChannelConfiguration')) then false() else true()"> If profile http://dashif.org/guidelines/dashif#mha1 is used, then schemeIdUri attribute shall be urn:mpeg:mpegB:cicp:ChannelConfiguration.</assert >
		</rule>
	</pattern>
	...
	<pattern name="SupplementalProperty element">
        ...
        <pattern name="EssentialProperty element">
