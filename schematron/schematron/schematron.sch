...
<schema xmlns="http://purl.oclc.org/dsdl/schematron" xmlns:dash="urn:mpeg:dash:schema:mpd:2011" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:cenc="urn:mpeg:cenc:2013" xmlns:dlb="http://www.dolby.com/ns/2019/dash-if" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" queryBinding='xslt2' schemaVersion='ISO19757-3'>
    ...
    <pattern>
        ...
        <rule context="dash:MPD">
            ...
            <assert test="if (@type = 'dynamic' and contains(@profiles, 'urn:mpeg:dash:profile:isoff-ext-live:2014')) then false() else true()">For ISO BMFF extended live profile, MPD@type shall be static.</assert>
            <assert test="if (@type = 'dynamic' and (contains(@profiles, 'urn:mpeg:dash:profile:isoff-ext-on-demand:2014') or contains(@profiles, 'http://dashif.org/guidelines/dash-if-ondemand'))) then false() else true()">For ISO BMFF extended on-demand profile, MPD@type shall be static.</assert>
        </rule>
    </pattern>
    <pattern>
        ...
        <rule context="dash:Period">
            ...
            <assert test="if (child::dash:SegmentList and ((@profiles and contains(@profiles, 'urn:mpeg:dash:profile:isoff-ext-live:2014')) or (not(@profiles) and contains(ancestor::dash:MPD/@profiles, 'urn:mpeg:dash:profile:isoff-ext-live:2014')))) then false() else true()">For ISO BMFF extended live profile, Period.SegmentList shall not be present.</assert>
            <!-- DASH 8.8.2 -->
            <assert test="if (contains(ancestor::dash:MPD/@profiles, 'urn:mpeg:dash:profile:isoff-ext-live:2014') and (count(child::dash:AdaptationSet[@contentType='video']) &gt; 1) and (count(descendant::dash:Role[@value='main'])=0)) then false() else true()">For ISO BMFF extended live profile: "If a Period contains multiple Adaptation Sets with value of the @contentType="video" then at least one Adaptation Set shall contain a Role el-ement $&lt;$Role scheme="urn:mpeg:dash:role:2011" value="main"&gt;"</assert>
            <!--    DASH 8.2.9 -->
            <assert test="if ((contains(ancestor::dash:MPD/@profiles, 'urn:mpeg:dash:profile:isoff-ext-on-demand:2014') or contains(ancestor::dash:MPD/@profiles, 'http://dashif.org/guidelines/dash-if-ondemand')) and (child::dash:SegmentList or child::dash:SegmentTemplate)) then false() else true()">For ISO BMFF extended on-demand profile, neither the Period.SegmentList element nor the Period.SegmentTemplate element shall be present for On-Demand profile, violated here.</assert>
            <!-- DASH 8.2.9 -->
            <assert test="if ((contains(ancestor::dash:MPD/@profiles, 'urn:mpeg:dash:profile:isoff-ext-on-demand:2014') or contains(ancestor::dash:MPD/@profiles, 'http://dashif.org/guidelines/dash-if-ondemand')) and (count(child::dash:AdaptationSet[@contentType='video']) &gt; 1) and (count(descendant::dash:Role[@value='main'])=0)) then false() else true()">For ISO BMFF extended on-demand profile: "If a Period contains multiple Adaptation Sets with value of the @contentType="video" then at least one Adaptation Set shall contain a Role el-ement $&lt;$Role scheme="urn:mpeg:dash:role:2011" value="main"&gt;"</assert>
        </rule>
    </pattern>
    <pattern>
    ...
    <pattern>
        ...
        <rule context="dash:ContentProtection">
            ...
            <assert test="if (contains(ancestor::dash:MPD/@profiles, 'http://dashif.org/guidelines/dash') and not(parent::dash:AdaptationSet)) then false() else true()">The ContentProtection descriptors shall always be present in the AdaptationSet element and apply to all contained Representations.</assert>
            <!--  R12.3  DASH-IF IOP v4.0-->
            <assert test="if (contains(ancestor::dash:MPD/@profiles, 'http://dashif.org/guidelines/dash') and (@schemeIdUri = 'urn:mpeg:dash:mp4protection:2011') and (@value= 'cenc') and not(parent::dash:AdaptationSet)) then false() else true()">The ContentProtection descriptor for the mp4 protection scheme with @schemeIdUri 'urn:mpeg:dash:mp4protection:2011' and @value 'cenc' shall be present in the AdaptationSet element.</assert>
            <!--  R12.3  DASH-IF IOP v4.2-->
            <assert test="if (contains(ancestor::dash:MPD/@profiles, 'http://dashif.org/guidelines/dash') and (@schemeIdUri = 'urn:mpeg:dash:mp4protection:2011') and not(@cenc:default_KID) ) then false() else true()">The ContentProtection Descriptor for the mp4protection scheme shall contain the attribute @cenc:default_KID.</assert>
        </rule>
    </pattern>
    <pattern>
        <title>AudioChannelConfiguration element</title>
